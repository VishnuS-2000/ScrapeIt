...
const {scrapWebsite,extractData, isValidURL}=require('../utils/scraper')
const insights=[]
/* Insight Controller */
...
const createInsight=async(req,res)=>{
    try{
        ...
        if(!isValidURL(req.body.url)) return res.status(400).json({result:{},message:'Not a valid URL'});
        ...
        insights.map((insight)=>{
            if(insight?.data.domain==result?.domain){
                existing=true
            }
        })
...
const deleteInsight=async(req,res)=>{
    try{
        ...
        insights.splice(deletedIndex,1)
